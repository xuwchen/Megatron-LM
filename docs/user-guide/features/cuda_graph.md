...
overwrite either bank. Before a fused weight-gradient CUDA graph writes `main_grad`, Megatron-FSDP
claims and validates that unit's planned bank.

The two banks are shared by all language-decoder layers. For each storage kind and dtype, each
bank's arena is sized to the maximum aggregate bucket size of any decoder FSDP unit. Vision encoder
buckets are not included in this plan and continue to use the dynamic allocator. Vision encoder TE
