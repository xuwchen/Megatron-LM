...
Three temporary environment controls cover behavior that does not yet have a public config field:
...
- `MEGATRON_CG_PLANNED_CAPTURE_RESIDENT=1` conservatively assigns a distinct planned weight and
  transpose-weight color to every same-offset graph bucket. This disables lifetime-based sharing
  for those buffers and increases memory use; it is intended as a compatibility diagnostic.
