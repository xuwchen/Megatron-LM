...
computation remains eager. During warmup, Megatron-FSDP observes complete unit lifetimes and, within
each parameter-and-gradient buffer (one model chunk), colors their overlap graph into two static
banks. All buckets belonging to one unit receive the same color. All-gather prefetch and
reduce-scatter queuing are throttled to at most two planned decoder units. A unit must also wait for
any earlier reduce-scatter reading its main-gradient bank before graph replay can reclaim it,
preserving the two-bank lifetime invariant.
...
`MEGATRON_CG_SKIP_BUFFER_ADDRESS_CHECK=1` disables replay-time parameter-address validation for
diagnostics only. It is unsafe for production because an address change can otherwise become silent
training corruption.
...
the supported MoE partial-capture setup, use
`--cuda-graph-modules attn moe_router moe_preprocess`; GDN capture under the `attn` scope is
experimental and requires `MEGATRON_GDN_TE_CUDA_GRAPH=1`.
