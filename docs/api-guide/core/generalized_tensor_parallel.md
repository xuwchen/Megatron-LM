...
- **Native-FP8 optimizer-state matching (Muon path).** The save-side dequantize (§3.3) hands DCP a *fresh* BF16 tensor, which breaks the id-based optimizer-param → model-`ShardedTensor` match for every native-FP8 GTP_remat weight. The dequantized copy carries a `_gtp_dequant_src` backlink to the live FP8 param, and `_backfill_gtp_sharded_param_map` reuses the model's **own** entry (backlink first, tagged-name second) — preserving its full offsets (expert axes included) and `replica_id`. Only truly-unmatched params (Mamba/GDN `in_proj`, a gathered+split factory) take the per-shard rebuild, which refuses expert-parallel params rather than emit EP-colliding shards.
...
>> Note: Mamba and GatedDeltaNet fused `in_proj` weights are special cases: their semantic split boundaries can cross GTP_remat shard boundaries. They **all-gather the GTP_remat shards** back to the logical TP-local size and strip the pad *before* splitting and saving, then merge, re-pad, and slice on load. Their checkpoint globals and split keys therefore match non-GTP runs and remain topology-independent. Caveats: the topology independence covers the **model weights only** — optimizer master/moment state still resolves through the per-shard rebuild in `_backfill_gtp_sharded_param_map`, so resuming optimizer state requires the same `gtp_remat_size` the checkpoint was saved with; the gathered entry replaces the per-shard one, so the direct-API `fully_sharded_model_space` optimizer sharding (identity-keyed on the model entry's tensor) does not support these params — the training flow always passes `dp_reshardable`/`fully_reshardable` and is unaffected; and tail alignment-pad rows are re-zeroed on load rather than round-tripped, so the first resume is not bit-identical in the (output-irrelevant) pad rows.
