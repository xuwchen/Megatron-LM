...
def _rope_config(**overrides):
    """Minimal config accepted by the public ``apply_rotary_pos_emb`` dispatcher.

    The wrapper routes through that dispatcher, which inspects fusion and mRoPE
    settings before handing off to the bshd/thd implementations these tests
    patch. Defaults keep every fast path disabled so the unfused call is
    exercised.
    """
    defaults = dict(
...
class _FakeCpGroup:
    """Identity-checkable CP group whose size the THD dispatcher queries."""
...
def test_apply_rope_fp32_forwards_bshd_options(monkeypatch):
    ...
    output = specs._apply_rope_fp32(
        tensor,
        freqs,
        _rope_config(rotary_interleaved=True),
        mscale=0.75,
...
def test_apply_rope_fp32_defaults_mla_interleaving_from_config(monkeypatch, config_value):
    ...
    specs._apply_rope_fp32(
        tensor, freqs, _rope_config(rotary_interleaved=False, multi_latent_attention=config_value)
    )
