...
@pytest.mark.parametrize("wrapper", [specs._apply_rope_fp32, specs._apply_rope_fp32_no_cp])
def test_apply_rope_fp32_wrappers_match_canonical_signature(wrapper):
    """The wrappers substitute ``apply_rotary_pos_emb`` positionally, so parameter names AND
    order must match exactly — a rename, reorder, or dropped parameter regresses silently
    otherwise (callers pass ``t``/``freqs`` positionally and the rest by keyword)."""
    assert list(inspect.signature(wrapper).parameters) == list(
        inspect.signature(rope_utils.apply_rotary_pos_emb).parameters
    )
