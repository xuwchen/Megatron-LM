...
def test_recipe_configuration_propagates_overlap_controls(monkeypatch):
    monkeypatch.setattr(gtp_module, "HAVE_TE", True)
    monkeypatch.setattr(gtp_module.torch.distributed, "is_initialized", lambda: False)

    gtp_module.configure_gtp_remat_from_recipe(weight_prefetch=False, async_reduction=False)

    assert gtp_module.GTP_CONFIG.weight_prefetch is False
    assert gtp_module.GTP_CONFIG.async_reduction is False


def test_recipe_configuration_applies_explicit_training_defaults(monkeypatch):
    monkeypatch.setattr(gtp_module, "HAVE_TE", True)
    monkeypatch.setattr(gtp_module.torch.distributed, "is_initialized", lambda: False)
    gtp_module.update_gtp_config(weight_prefetch=False, async_reduction=False)

    gtp_module.configure_gtp_remat_from_recipe(weight_prefetch=True, async_reduction=True)

    assert gtp_module.GTP_CONFIG.weight_prefetch is True
    assert gtp_module.GTP_CONFIG.async_reduction is True


def test_recipe_configuration_preserves_direct_integration_controls(monkeypatch):
    monkeypatch.setattr(gtp_module, "HAVE_TE", True)
    monkeypatch.setattr(gtp_module.torch.distributed, "is_initialized", lambda: False)
    gtp_module.update_gtp_config(weight_prefetch=False, async_reduction=False)
