...
def _worker_pre_init_tp_padding(rank, world_size, port, tp_size, gtp_remat_size):
    ...
    try:
        ...
        # gtp_ctx layout: (gtp_remat_group, pad_length, logical_out_features).
        ctx_group, pad_length, logical = gtp_ctx
        assert pad_length == 32, f"rank {rank}: expected pad_length 32, got {pad_length}"
        assert ctx_group.size() == gtp_remat_size and logical == out_features
    finally:
...
def _worker_presharded_bias(rank, world_size, port, tp_size, gtp_remat_size):
    ...
    torch.manual_seed(0)
    tp_group, gtp_remat_group, tp_rank, _ = _build_groups(
        rank, world_size, tp_size, gtp_remat_size
    )
    batch = 4
    dtype = torch.bfloat16
    ...
    try:
        ...
        # Module-level forward-gather gate for the companion TE integration must be stamped.
        assert no_bias_layer.gtp_remat_size == gtp_remat_size
        ...
        for parallel_mode, logical_out_features, out_split_size, input_features in cases:
            ...
            _gtp_attach_post_init(
                layer, gtp_ctx, out_split_size=out_split_size
            )
...
class TestTPGTPReplicatedBias:
    ...
    def test_row_and_column_bias_forward_backward(self, tp_size, gtp_remat_size):
        ...
        _requires_multi_gpu(world_size)
        _run_distributed(
            _worker_presharded_bias, world_size, tp_size, gtp_remat_size
        )
...
def _worker_prefetch_boundary(rank, world_size, port, tp_size, gtp_remat_size):
    ...
    torch.manual_seed(0)
    _, gtp_remat_group, _, _ = _build_groups(
        rank, world_size, tp_size, gtp_remat_size
    )
...
class TestTPGTPPrefetchBoundary:
    ...
    def test_embedding_starts_new_prefetch_segment(self, tp_size, gtp_remat_size):
        ...
        _requires_multi_gpu(world_size)
        _run_distributed(
            _worker_prefetch_boundary, world_size, tp_size, gtp_remat_size
        )
