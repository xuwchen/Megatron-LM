...
def _worker_gated_fused_microbatches(rank, world_size, port):
    ...
    # Backward: the wgrad must land on the STORAGE layout ([gate_i | up_i] per shard), i.e.
    # the exact inverse of the consume-side de-interleave. lr=0 A/Bs and every grad-norm
    # probe are permutation-invariant, so only an elementwise check can see this.
    ...
    for layer, ref in zip(layers, refs):
        ...
        ref_leaf = ref.detach().clone().float().requires_grad_(True)
    ...
    chunk = out_f // (2 * world_size)
    for i, (layer, ref_leaf) in enumerate(zip(layers, ref_leaves)):
        # Map the reference grad (in consumed [all gate | all up] layout) back to storage.
        storage = (
            ref_leaf.grad.view(2, world_size, chunk, in_f).transpose(0, 1).reshape(out_f, in_f)
        )
        want = storage[rank * shard_rows : (rank + 1) * shard_rows]
        got = layer.weight.main_grad.float()
        wrong = ref_leaf.grad[rank * shard_rows : (rank + 1) * shard_rows]  # un-permuted chunk
        assert torch.allclose(got, want, atol=2e-1, rtol=2e-2), (
            f"layer {i}: GLU wgrad landed on the wrong storage rows "
            f"(max_diff={(got-want).abs().max():.4f}); the consume-side de-interleave has no "
            f"wgrad-side inverse, so shard {rank} accumulated another shard's gradient rows."
        )
        if world_size > 1:
            assert not torch.allclose(got, wrong, atol=2e-1, rtol=2e-2), (
                "test lost its discrimination: the storage-layout and consumed-layout "
                "references coincide; enlarge the layer so the permutation is visible"
            )
