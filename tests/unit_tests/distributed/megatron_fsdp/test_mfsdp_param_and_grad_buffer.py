...
def test_planned_allocator_separates_overlapping_and_capture_resident_lifetimes(
    cpu_global_memory_buffer,
):
    ...
    capture_resident = PlannedBucketAllocator(
        "capture_resident", _TrackingBucketAllocator(), _allocator_param_groups(0, 1)
    )
    for bucket_id in (0, 1):
        capture_resident.allocate(bucket_id, 8, torch.float32, torch.device("cpu"))
        capture_resident.free(bucket_id)
    capture_resident.freeze_plan([0, 1], capture_resident=True)

    assert capture_resident._plan[0] != capture_resident._plan[1]
