...
def wrap_model_chunks_with_ddp(
    ...
    *,
    ...
    for chunk_idx, (chunk, layout, disable_bucketing) in enumerate(
        ...
        # Planned FSDP buffers live in a process-global memory registry. Give every
        # virtual-pipeline model chunk a rank-stable namespace so independent allocators
        # cannot resolve to the same storage.
