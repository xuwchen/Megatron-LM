...
def calc_params_l2_norm(model, force_create_fp32_copy=False):
    ...
    # Four parameter categories, each split into regular data, distributed-optimizer
    # shards, and (for the non-sharded path) local DTensor shards. The latter must be
    # reconstructed over their DeviceMesh group before model-parallel reductions.
    params_data = []
    sharded_params_data = []
    dtensor_params_data = []
    gtp_params_data = []
    gtp_sharded_params_data = []
    dtensor_gtp_params_data = []
    moe_params_data = []
    moe_sharded_params_data = []
    dtensor_moe_params_data = []
    moe_gtp_params_data = []
    moe_gtp_sharded_params_data = []
    dtensor_moe_gtp_params_data = []

    dense_dtensor_group = None
    expert_dtensor_group = None
    ...
    for model_chunk in model:
        for param in model_chunk.parameters():
            ...
            # Preserve latest-dev DTensor semantics: norms are computed from local shards and
            # summed over the tensor's DeviceMesh group. Discover the group before duplicate
            # filtering so every peer enters the same collectives, including peers that add zero.
            if is_expert:
                param_dtensor_group = get_data_parallel_group_if_dtensor(
                    param, expert_dtensor_group
                )
                if param_dtensor_group is not None:
                    expert_dtensor_group = param_dtensor_group
            else:
                param_dtensor_group = get_data_parallel_group_if_dtensor(
                    param, dense_dtensor_group
                )
                if param_dtensor_group is not None:
                    dense_dtensor_group = param_dtensor_group
            is_dtensor = param_dtensor_group is not None
            ...
            # GTP/EGTP shards are unique. Unwrapped parameters are replicated on their
            # corresponding remat axis and are counted on rank zero only; a later collective
            # over that axis both deduplicates and makes the resulting norm peer-consistent.
            ...
            if is_expert:
                ...
                if is_gtp:
                    if is_sharded:
                        moe_gtp_sharded_params_data.append(data)
                    elif is_dtensor:
                        dtensor_moe_gtp_params_data.append(data)
                    else:
                        moe_gtp_params_data.append(data)
                else:
                    if is_sharded:
                        moe_sharded_params_data.append(data)
                    elif is_dtensor:
                        dtensor_moe_params_data.append(data)
                    else:
                        moe_params_data.append(data)
            else:
                if param_is_not_shared(param):
                    ...
                    if is_gtp:
                        if is_sharded:
                            gtp_sharded_params_data.append(data)
                        elif is_dtensor:
                            dtensor_gtp_params_data.append(data)
                        else:
                            gtp_params_data.append(data)
                    else:
                        if is_sharded:
                            sharded_params_data.append(data)
                        elif is_dtensor:
                            dtensor_params_data.append(data)
                        else:
    ...
    # Reconstruct local DTensor shards over their DeviceMesh groups exactly once. Optimizer-
    # sharded main_params intentionally stay out of these buckets: their DP/EDP reductions below
    # already reconstruct those shards, matching latest-dev's force-copy/no-copy behavior.
    dtensor_params_norm_2 = _compute_norm_2(dtensor_params_data)
    dtensor_gtp_norm_2 = _compute_norm_2(dtensor_gtp_params_data)
    if dense_dtensor_group is not None:
        _sum_reduce(dtensor_params_norm_2, dense_dtensor_group)
        _sum_reduce(dtensor_gtp_norm_2, dense_dtensor_group)
    params_norm_2 += dtensor_params_norm_2
    gtp_norm_2 += dtensor_gtp_norm_2

    dtensor_moe_norm_2 = _compute_norm_2(dtensor_moe_params_data)
    dtensor_moe_gtp_norm_2 = _compute_norm_2(dtensor_moe_gtp_params_data)
    if expert_dtensor_group is not None:
        _sum_reduce(dtensor_moe_norm_2, expert_dtensor_group)
        _sum_reduce(dtensor_moe_gtp_norm_2, expert_dtensor_group)
    moe_norm_2 += dtensor_moe_norm_2
    moe_gtp_norm_2 += dtensor_moe_gtp_norm_2
    ...
    # expert_model_parallel = ETP×EP×PP and deliberately excludes EGTP. Reduce the ENTIRE
    # expert contribution over EGTP: wrapped weights are distinct shards and must be summed,
    # while unwrapped expert params were retained only on EGTP rank zero and must be broadcast.
    # Reducing only the wrapped buckets would leave unwrapped expert params with different norms
    # on EGTP peers; keeping them on every peer would double-count once this reduction is applied.
    moe_total_norm_2 = (
        moe_norm_2 + moe_sharded_norm_2 + moe_gtp_norm_2 + moe_gtp_sharded_norm_2
    )
    _sum_reduce(moe_total_norm_2, mpu.get_expert_gtp_weight_remat_group())
