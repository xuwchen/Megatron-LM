...
def get_rng_state(
    ...
    ``dp_group`` identifies the ranks that gather and index RNG state, while
    ``dp_cp_group`` identifies checkpoint replicas. ``key_prefix`` namespaces
    the sharded RNG object for models that checkpoint multiple disjoint grids.
    """
    args = get_args()
    rng_state = {
    ...
    if (
        ...
        rng_state_list = [None for _ in range(dp_world_size)]
...
def save_checkpoint(
    ...
    Args:
        ...
        rng_state_key_prefix: Prefix used to namespace the sharded RNG-state key
    """
    start_ckpt = time()
    args = get_args()
