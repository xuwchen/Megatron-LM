...
def classify_gtp_chains(model) -> None:
    ...
    for name, param in model.named_parameters():
        ...
        if getattr(param, "_gtp_multi_use", False):
            if param.prev_w is not None or param.next_w is not None:
                # A boundary marked after its links were wired cannot be un-linked here: its
                # neighbours already point at it and would keep feeding its single ticket slot.
                raise RuntimeError(
                    f"classify_gtp_chains: multi-use boundary param {name} is already linked "
                    "into a prefetch chain. mark_gtp_multi_use_boundary() must run at module "
                    "construction, before the first forward wires the chain links."
                )
...
class GTPShardedParam(torch.nn.Parameter):
    ...
    def all_gather_and_prefetch(self, fwd: bool = True, nvtx_label: str = None):
        ...
        if not self.prefetch_initialized:
            ...
            # A boundary is not a chain member, so it must not become the anchor the next
            # weight links to — otherwise the chain reforms through it on the following link.
            if not is_boundary:
