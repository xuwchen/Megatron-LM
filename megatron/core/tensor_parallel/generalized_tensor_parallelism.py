...
def mark_gtp_multi_use_boundary(param) -> None:
    ...
    handled synchronously: gather on demand, reduce-scatter each wgrad on the spot, accumulate
    into main_grad immediately, and clear every slot afterwards so the next use starts clean.
...
def classify_gtp_chains(model) -> None:
    ...
    for name, param in model.named_parameters():
        ...
        if getattr(param, "_gtp_multi_use", False):
            # A boundary param must never be linked: prev_w/next_w drive both the AG prefetch
            # and the wgrad RS cascade, and a cascade that runs through a param used twice
            # would finalize it from the wrong side. Enforce the opt-outs the marker set, in
            # case construction order let something re-enable them.
            param._need_weight_prefetch = False
            param._need_weight_prefetch_bwd = False
            if param.next_w is not None or param.prev_w is not None:
                raise RuntimeError(
                    f"classify_gtp_chains: {name} is marked as a GTP multi-use boundary but is "
                    "already linked into a prefetch chain. Mark it before the first forward."
                )
...
def _init_gtp_runtime_attrs(obj):
    ...
    # Multi-use hard boundary: True when ONE parameter is consumed more than once per
    # forward (MTP reuses the shared embedding and the shared lm-head), so its AG/RS state
    # slots below would be clobbered by the second use. Set explicitly at construction via
...
class GTPShardedParam(torch.nn.Parameter):
    ...
    def all_gather_and_prefetch(self, fwd: bool = True, nvtx_label: str = None):
        ...
        if not self.prefetch_initialized:
            ...
            # A weight that cannot be prefetched starts a new chain segment. In multimodal
            ...
            if last_w is not None and last_w.next_w is None and self._need_weight_prefetch:
    ...
    def wgrad_reduce_scatter(self, wgrad, nvtx_label=None):
        ...
        # The synchronous collective returns shard-sized output tensors; only the original
        # full-wgrad scratch buffers came from _wgrad_pool_get() and may be recycled there.
