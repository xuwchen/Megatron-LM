...
def _layer_is_graphable(layer, config):
    ...
    if isinstance(layer, TransformerLayer):
        if not getattr(layer.self_attention, 'supports_te_cuda_graph', True):
            # e.g. GatedDeltaNet: the FLA kernels replay different values under
            # TE partial CUDA graphs, so keep the whole layer eager.
            return False
        if CudaGraphModule.attn in config.cuda_graph_modules and not (
