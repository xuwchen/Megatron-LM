...
class _CudagraphReplayNode(torch.autograd.Function):
    ...
    def backward(ctx, *grads):
        ...
        # GTP's grad-ready hook is silenced during capture. Replay its hooks on each
        # parameter's RS stream, after the runner stream has completed the captured Phase 2
        # main_grad accumulation, so DDP never reduces a stale gradient.
        if runner.gtp_remat:
        ...
        # regular wgrads as autograd may launch the next graph before they are accumulated.
        # GTP params have already reduce-scattered into main_grad and must not feed a second
        # gradient through autograd.
        dgrads = runner.static_grad_inputs[: runner.num_dgrads]
        wgrads = (
            None if getattr(param, 'is_gtp_weight_remat', False) else grad.clone()
            for param, grad in zip(
                runner.params_to_backprop,
                runner.static_grad_inputs[runner.num_dgrads :],
            )
        )
...
class _CudaGraphRunner(torch.nn.Module):
    ...
    def __init__(
        ...
        if hasattr(self.base_module, "config") and isinstance(
            ...
            self.fp8_runtime_enabled = None
            self.fp4_runtime_enabled = None
            dense_gtp_remat = self.base_module.config.gtp_weight_remat_size > 1
            expert_gtp_remat = self.base_module.config.expert_gtp_weight_remat_size > 1
            # Routed experts use EGTP independently of dense GTP. Both need the same runner,
            # capture, side-stream, and post-replay DDP-hook choreography.
            self.gtp_remat = dense_gtp_remat or expert_gtp_remat
            ...
            if self.gtp_remat:
                ...
                if expert_gtp_remat:
                    self._register_gtp_side_streams(pg_collection.expt_gtp_remat)
    ...
    def create_bwd_graph(self):
        ...
        with torch.cuda.graph(self.bwd_graph, pool=self.mempool):
            ...
            if self.gtp_remat:
                ...
                wait_async_comms(GTPChain.GRAPHED.value, skip_rs=True)
                # fwd_side_streams is exactly the AG stream set registered for the active axes;
                # using it avoids silently dropping EGTP when dense GTP is disabled.
                self._wait_side_streams(self.fwd_side_streams)
                ...
                wait_async_comms(GTPChain.GRAPHED.value, finalize_after_drain=True)
