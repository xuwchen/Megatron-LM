...
def _gtp_attach_post_init(module, gtp_ctx, is_grouped=False, *, out_split_size):
    ...
    # Module-level forward-gather gate read by the companion TE distributed-weight integration
    # (in-tree readers use the per-param ``weight.gtp_remat_size`` set during attach instead).
    # Mirrors the convention VocabParallelEmbedding and the GTP test utilities follow.
    module.gtp_remat_size = gtp_remat_group.size()
...
def _init_gtp_remat_context(
    ...
    *,
    ...
    _gtp_attach_post_init(
        module, gtp_ctx, is_grouped=is_grouped, out_split_size=out_split_size
    )
