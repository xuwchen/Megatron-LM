    ...
    def __init__(
        ...
        for bucket_id, param_group in enumerate(fsdp_param_groups):
            ...
            if unit_id in (None, -1):
                ...
                continue
            bucket_offsets[bucket_id] = len(unit_buckets[unit_id])
            unit_buckets[unit_id].append(bucket_id)
        self._all_unit_buckets = dict(unit_buckets)
    ...
    def _enforce_double_buffer_limit(self, add_buckets):
        ...
        if len(double_buf_units) > 2:
            raise RuntimeError(
                ...
                f"Current double-buffer units: {double_buf_units}."
            )

        # Keep the largest queue suffix that, together with the incoming
        # buckets, uses at most two planned units. Dynamic Vision entries count
        ...
        for _, _, bucket_id in reversed(self.grad_reduce_queue):
            ...
            if (
                ...
                and fsdp_unit_id not in double_buf_units
                and len(double_buf_units) == 2
            ):
                break
            if fsdp_unit_id in self.buffer.double_buf_units:
                double_buf_units.add(fsdp_unit_id)
