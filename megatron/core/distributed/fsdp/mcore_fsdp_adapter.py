...
def _validate_cuda_graph_config(config, ddp_config):
    ...
    if getattr(config, "cuda_graph_warmup_steps", 0) < 1:
        raise ValueError(
            "Megatron-FSDP planned double buffering with "
            "cuda_graph_impl='transformer_engine' requires "
            "cuda_graph_warmup_steps >= 1 to observe eager decoder-unit bucket lifetimes."
        )
