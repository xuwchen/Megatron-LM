...
def has_nvrx_async_support() -> bool:
    ...
    # Capability probe: an nvidia-resiliency-ext older than NVRX_MIN_VERSION
    # means "no async support", not an import-time crash. Asserting here breaks
    # every module that transitively imports dist_checkpointing inside
    # containers shipping an older nvrx, even when async checkpointing is
    # never used.
