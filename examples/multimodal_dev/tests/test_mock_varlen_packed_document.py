...
class TestPackedDocumentPlanner:
    ...
    def test_full_pool_sweep_holds_every_invariant_and_aggregate(self):
        ...
        assert generator.total_supervised_tokens == sum(
            window.supervised_tokens for window in windows
        )
        assert generator.total_documents == sum(len(window.documents) for window in windows)
        assert generator.total_logical_tokens == sum(window.logical_tokens for window in windows)
        assert generator.total_vision_tokens == sum(window.vision_tokens for window in windows)
