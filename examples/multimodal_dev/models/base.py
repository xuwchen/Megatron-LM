...
class MultimodalModel(MegatronModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Rank-symmetric entry contract: _vision_forward's lockstep
        # all-reduce assumes every rank in the sharding group takes this
        # branch together. Eager samples always carry pixel_values (an
        # empty [0, dim] tensor when image-free) and streaming ranks all
        # own a noise pool, so the condition is uniform across ranks.
