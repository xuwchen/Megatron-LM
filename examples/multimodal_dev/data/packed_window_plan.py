...
# The frozen parity calibration: window-level statistics of this profile
# match the m-lm-mistralai parity_10937 production windows (87,496 real
# 4K training windows; fit seed 1234 @200K windows, independent seed
# 987654 @600K). Every value is traceable to one of three origins:
#
#   FITTED  (coordinate fit against parity window statistics):
#     text_only_document_probability 0.735  -> text-only window fraction .553
#     image_poisson_rate_per_1k_text_tokens 1.76 -> images/1K tokens .442
#     image_density_gamma_shape 0.46        -> bearing-share p99 .84 tail
#     image_sizes weights                   -> tokens/image p50 154 (exact),
#                                              mean 258, p99 1110, max 1369
#   DERIVED (closed-form solutions of round targets):
#     component weights 99.482/0.518 = exact solution of a 25% long-doc
#       text-token share: p/(1-p) = s*E[T|short]/((1-s)*E[T|long]) with the
#       post-truncation means 2048/131072
#     max_boundary_fill_fraction 0.005 = ~4x headroom over the worst
#       measured production fill (0.11% at 4K, lower at larger S)
#     plan_pool_windows "auto" = max(2048, ceil(2^26 / seq_length)):
#       >= 64Mi plan tokens at any S puts ~110 expected long documents in
#       the pool under the parity mixture, pinning the realized long-doc
#       token share near nominal (measured seed drift ~+-9% relative,
#       vs ~+-40% for a flat 2048 windows at 4K); the 2048 floor keeps
#       long-context pools at the proven cost bound (128K: 2048 windows,
#       seconds to build). An explicit integer is an upper bound, clamped
#       by the split's sample count (e.g. the 128K qualification pins 200
#       = one exact pass over the pool)
#     plan_seed 1234 = the calibration-snapshot seed. The window LAYOUT
#       (plan pool) is seeded by this key, independently of --seed, so the
#       workload shape is a constant of the profile — training seeds vary
#       token/pixel CONTENT only, and realized pool statistics match the
#       fixed-seed snapshots. Override the key to study layout variance
#   STRUCTURAL HYPOTHESES (round design choices; window-level consequences
#   validated by the parity fit):
#     short component: web-corpus scale (median ~1K, mode ~230 under
#       mean 2048 / sigma 1.2), min 64, max 32767 (disjoint from long)
#     long component: 128K-context scale (mean 131072 / sigma 0.8),
#       "long" = beyond 32768, max 524288 = 4x mean tail room
#     image_sizes resolutions: the Qwen3.5-VL processor ladder (multiples
#       of 32), every non-square size in both orientations for MRoPE
#
# Tests lock this dict against drift (test_defaults_are_the_frozen_
# parity_profile); the fixed-seed snapshot JSONs under the toolkit's
# docs/mcore/packed_window_snapshots/ echo the same values.
...
CONTEXT_SCALED_MAX_SEQ_LENGTH = 524288


def context_scaled_doc_length(seq_length: int) -> dict[str, Any]:
    """Expand the opt-in ``"doc_length": "context_scaled"`` coverage policy.

    An engineering interpolation (a coverage POLICY, not a fitted law):
    the long-document text-token share grows by 5 percentage points per
    doubling of the window,

        s(S) = clamp(0.25 + 0.05 * log2(S / 4096), 0.25, 0.60)

    anchored at 4K = 0.25 (the parity-era coverage hypothesis under which
    the 4K window-level parity fit closed), 128K = 0.50 (the qualified
    128K recipe, between Fu et al.'s minimum-sufficient upsampling and
    ProLong's 60/40), and capped at 0.60 from 512K on (ProLong-inspired:
    short data must remain a large fraction). The long component stays
    the corpus's real long-document population (32768..524288, mean
    131072) at every S — the policy moves the SHARE, never the documents
    themselves. Beyond S = 524288 there is no anchor to interpolate
    toward: pass explicit components instead.

    Weights are quoted to three decimals so the 4K and 128K expansions
    are byte-identical to the frozen parity profile and the qualified
    128K recipe respectively.
    """
    seq_length = int(seq_length)
    if seq_length <= 0:
        raise ValueError(f"seq_length must be positive, got {seq_length}.")
    if seq_length > CONTEXT_SCALED_MAX_SEQ_LENGTH:
        raise ValueError(
            "context_scaled doc_length is defined for seq_length <= "
            f"{CONTEXT_SCALED_MAX_SEQ_LENGTH} (no anchor exists beyond 512K); "
            "pass explicit doc_length components for longer windows."
        )
    share = min(0.60, max(0.25, 0.25 + 0.05 * math.log2(seq_length / 4096.0)))
    mean_short, mean_long = 2048.0, 131072.0
    p_long = share * mean_short / ((1.0 - share) * mean_long + share * mean_short)
    long_weight = round(100.0 * p_long, 3)
    short_weight = round(100.0 - 100.0 * p_long, 3)
    return {
        "components": [
            {
                "name": "short",
...
class PackedWindowPlanGenerator:
    ...
    def __init__(
        ...
        *,
        ...
        # Feasibility is a property of the buckets that can actually be drawn:
        # a zero weight disables a bucket, so an oversized disabled bucket must
        # not block an otherwise valid window size.
        largest_atom = 1 + max(
            merged for merged, weight in zip(bucket_merged_tokens, weight_array) if weight > 0
        )
        ...
        for index, component in enumerate(components):
            ...
            names.append(name)
            unknown_fields = set(component) - {"name", "weight", "min", "max", "mean", "sigma"}
            if unknown_fields:
                # A typoed field would otherwise be silently ignored and the
                # benchmark would run a distribution the user did not write.
                raise ValueError(
                    f"doc_length component {name!r} has unknown field(s) "
                    f"{sorted(unknown_fields)}; allowed: name, weight, min, max, mean, sigma."
                )
            weight = _require_number(component["weight"], what=f"{name}.weight", minimum=0.0)
            weights.append(weight)
            minimum = _require_integer(component["min"], what=f"{name}.min")
            maximum = _require_integer(component["max"], what=f"{name}.max")
