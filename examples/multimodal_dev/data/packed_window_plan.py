...
Generative model: documents are drawn i.i.d. (text length from a disjoint
short/long truncated-lognormal mixture; per-doc image density from a Gamma
prior; image counts Poisson in the text length; image sizes from weighted
resolution buckets) and concatenated into a stream. Window ``idx`` is the
...
class _TruncatedLognormal:
    ...
    def __init__(self, *, mean: float, sigma: float, minimum: int, maximum: int) -> None:
        ...
        if self.minimum == self.maximum:
            self.mu = self._log_min
            self._cdf_lo, self._cdf_hi = 0.0, 1.0
            return
...
class PackedWindowPlanGenerator:
    ...
    Args:
        ...
        config: dict with ``doc_length`` (``short``/``long`` components with
            post-truncation ``mean``, ``sigma``, ``min``, ``max``, plus
            ``long_component_text_token_share``), ``p_text``, and
            ``image_density`` (``mean_per_text_token``, ``gamma_shape``).
    ...
    def __init__(
        ...
        *,
        ...
        short = doc_length["short"]
        long_ = doc_length["long"]
        self._short = _TruncatedLognormal(
            mean=_require_number(short["mean"], what="short mean", minimum=1),
            sigma=_require_number(short["sigma"], what="short sigma"),
            minimum=int(short["min"]),
            maximum=int(short["max"]),
        )
        self._long = _TruncatedLognormal(
            mean=_require_number(long_["mean"], what="long mean", minimum=1),
            sigma=_require_number(long_["sigma"], what="long sigma"),
            minimum=int(long_["min"]),
            maximum=int(long_["max"]),
        )
        if int(short["max"]) >= int(long_["min"]):
            raise ValueError("short/long doc-length components must be disjoint.")
        share = _require_number(
            doc_length["long_component_text_token_share"],
            what="long_component_text_token_share",
            minimum=0.0,
        )
        if share >= 1.0:
            raise ValueError("long_component_text_token_share must be < 1.")
        if share == 0.0:
            self.p_long = 0.0
        else:
            ratio = share * self._short.mean / ((1.0 - share) * self._long.mean)
            self.p_long = ratio / (1.0 + ratio)

        self.p_text = _require_number(config["p_text"], what="p_text", minimum=0.0)
        if self.p_text > 1.0:
            raise ValueError("p_text must be in [0, 1].")
        density = config["image_density"]
        self.density_mean = _require_number(
            density["mean_per_text_token"], what="image_density.mean_per_text_token"
        )
        if self.density_mean <= 0:
            raise ValueError("image_density.mean_per_text_token must be positive.")
        self.gamma_shape = _require_number(density["gamma_shape"], what="gamma_shape")
        if self.gamma_shape <= 0:
            raise ValueError("image_density.gamma_shape must be positive.")
    ...
    def _draw_doc(self, doc_id: int):
        ...
        sampler = self._long if component_rng.random() < self.p_long else self._short
        ...
        count = int(self._rng(doc_id, _STREAM_DOC_COUNT).poisson(lam * text_len))
        if count == 0:
            return text_len, (), ()
