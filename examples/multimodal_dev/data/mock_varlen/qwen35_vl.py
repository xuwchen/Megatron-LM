...
# Ceiling on seq_length x plan_pool_windows (the auto rule lands two orders
# of magnitude below it; this only bounds explicit profiles).
_MAX_PLAN_TOKENS = 1 << 32

# Per-sample content-stream ids for _seed_sequence. Disjoint by design from
# the plan kernel's per-document layout streams (21-23 in
# packed_document.py): content and layout live in different seed
...
def resolved_profile_sha256(profile: dict[str, Any]) -> tuple[str, str]:
    """Canonical resolved-profile serialization and its sha256.

    THE artifact convention (sort_keys, compact separators) — provider
    logs, simulator reports, and evidence tables must all go through
    this function so the hash can never silently diverge.
    """
...
def train_valid_test_varlen_datasets_provider(
    ...
    # (with its hash) plus the pool maxima as the launch artifact. The scan
    # applies to explicit configs exactly as to the default; changing
    # plan_seed or the distribution re-verifies automatically.
    resolved_json, resolved_hash = resolved_profile_sha256(config)
    # The full resolved JSON is printed BEFORE any guard verdict so a
    # failing launch still leaves a reproducible artifact (the profile that
    # produced the hash), not just the hash and the offending maxima.
    _print_rank0(f"[mock_varlen] resolved profile sha256={resolved_hash}: {resolved_json}")
    ...
    for split_name, dataset in zip(("train", "valid", "test"), datasets):
        ...
        if patch_budget is not None and plan.pool_max_raw_patches > int(patch_budget):
            raise ValueError(
                ...
                f"raise the budget. Resolved profile sha256={resolved_hash}: "
                f"{resolved_json}"
            )
