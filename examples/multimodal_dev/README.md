...
`--recompute-vision`; the 65,536 raw-patch guard bounds the vision
payload only and does not by itself guarantee memory runnability; MTP
is rejected at this entry): with no
...
- `--multimodal-varlen-mock-dataset-config-json`: the single, optional
  ...
  resolved profile (with its sha256) plus the pool
